    ...
    const handleCreateItem = (formData, type) => {
        ...
        switch (type) {
          ...
          case 'expense':
            ...
            // Ensure the category exists in budgets
            const budgetForCategory = budgets.find((budget) => budget.category === formData.category);
            if (!budgetForCategory) {
              toast.error(`No budget found for the category "${formData.category}".`);
              return; // Exit early to prevent API call
            }
            break;
